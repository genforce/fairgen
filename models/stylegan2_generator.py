...
class StyleGAN2Generator(nn.Module):
    ...
    def __init__(self,
        ...
        self.run_device = 'cuda' if use_cuda else 'cpu'
        self.cpu_device = 'cpu'
    ...
    def forward(self,
        ...
        if latent_space_type == 'Z':
            ...
            mapping_results = self.mapping(zs, label)
            w = mapping_results['w']
        ...
        # Generate from W space.
            ...
            wp = self.truncation(ws, trunc_psi, trunc_layers)
            results['w'] = self.get_value(ws)
        ...
        # Generate from W+ space.
            ...
            wp = self.truncation(wp, trunc_psi, trunc_layers)
            results['wp'] = self.get_value(wp)

        synthesis_results = self.synthesis(wp, randomize_noise)

        synthesis_results['image'] = self.postprocess(synthesis_results['image'])

        results = {**synthesis_results, **results}

        for key in results:
            if torch.is_tensor(results[key]):
                results[key] = self.get_value(results[key])
